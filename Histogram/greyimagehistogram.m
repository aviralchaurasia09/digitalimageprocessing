...
figure(2)
Im1G_vector = Im1(:); % converts to a single vector
subplot(2, 1, 1); imhist(Im1); ylabel('Grey Image Histogram');
...
Im1G_contrast = imadjust(Im1, stretchlim(Im1, [0.05, 0.95]),[]);

figure(3);
subplot(3, 2, 1); imshow(Im1); xlabel('Original Image');
subplot(3, 2, 2); imhist(Im1); ylabel('Original Histogram');
subplot(3, 2, 3); imshow(h2), xlabel('Histogram Slide Image');
subplot(3, 2, 4); imhist(h2), ylabel('Histogram Slide Histogram');
subplot(3, 2, 5); imshow(uint8(Im1_contrast)); xlabel('Contrast Stretching')
subplot(3, 2, 6); imhist(Im1_contrast); ylabel('Contrast Stretching');
